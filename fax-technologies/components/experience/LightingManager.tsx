...
const LIGHTING_PROFILES: Record<string, { primary: string; secondary: string; intensity: number }> = {
  ...
  purple: { primary: "#a855f7", secondary: "#ec4899", intensity: 4.2 },
  ...
  gold: { primary: "#eab308", secondary: "#f59e0b", intensity: 4.0 },
  cyan: { primary: "#06b6d4", secondary: "#3b82f6", intensity: 3.5 },
  minimal: { primary: "#64748b", secondary: "#475569", intensity: 2.0 },
};
...
export function LightingManager() {
  ...
  useFrame(() => {
    ...
    if (light1Ref.current) {
      ...
      light1Ref.current.intensity = profile.intensity;
    }
    if (light2Ref.current) {
      light2Ref.current.color.lerp(new Color(profile.secondary), MOTION_TOKENS.lerp.lightingShift);
      light2Ref.current.intensity = profile.intensity * 0.7;
    }
  });
